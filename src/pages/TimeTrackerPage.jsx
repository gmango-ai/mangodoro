  ...
  return (
    <div>
      ...
      <div className={`sticky top-0 z-30 backdrop-blur-md border-b ${
        dark ? "bg-[var(--color-surface)] border-[var(--color-border-light)]" : "bg-white/70 border-slate-200/70"
      }`}>
      ...
      {/* Content — each tab renders its existing page component as-is so
          ...
          Quick-clock UX lives inside LogPage's auto mode now, so we no
          longer mount a separate SalaryClockCard up here. */}
      <div>
        {tab === "log" && <LogPage />}
