...
export function useFeaturedSpeaker(speaking, { decayMs = 2500 } = {}) {
  ...
  useEffect(() => {
    ...
    // Nobody speaking right now: hold the current feature, then release after the
    // decay window if it's still quiet. Don't reset the timer on unrelated
    // re-renders — only arm it once per silence.
    if (!timerRef.current) {
