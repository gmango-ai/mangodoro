...
// Bigger, comfier targets: 48px on touch, 40px with a mouse — the `[&_svg]`
// variant scales each button's icon in step so the padding stays balanced.
export const TOOL_BTN_SIZE = WB_TOUCH
  ? "w-12 h-12 [&_svg]:w-[22px] [&_svg]:h-[22px]"
  : "w-10 h-10 [&_svg]:w-[19px] [&_svg]:h-[19px]";
...
export const CARET_CLS = WB_TOUCH
  ? "w-7 h-12 -ml-1.5 rounded-full flex items-center justify-center"
  : "absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full flex items-center justify-center shadow";
