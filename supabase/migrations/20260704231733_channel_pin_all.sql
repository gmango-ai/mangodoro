-- Pin a channel to the top for EVERYONE (admin-curated, e.g. #announcements) —
-- a channel-level flag (distinct from the per-user pinned_at on channel_read_state).
--
-- Fresh timestamp — rewrites list_my_conversations from 20260704223239.

alter table public.conversations add column if not exists pinned_all boolean not null default false;

-- Extend set_channel_meta with p_pinned_all (drop the 9-arg version first).
drop function if exists public.set_channel_meta(uuid, text, text, text, text, boolean, int, boolean, boolean);
create or replace function public.set_channel_meta(
  ...
  p_pinned_all boolean default null
...
begin
  ...
  update public.conversations set
    ...
    pinned_all     = coalesce(p_pinned_all, pinned_all)
   where id = p_conversation_id;
end; $$;
grant execute on function public.set_channel_meta(uuid, text, text, text, text, boolean, int, boolean, boolean, boolean) to authenticated;

-- Inbox: return pinned_all too.
...
language sql stable security definer set search_path = '' as $$
  ...
  select c.id, c.kind, c.title, c.last_message_at, a.last_read_at,
    ...
    c.color, c.archived_at, c.retention_days, c.allow_images, c.force_notify, c.pinned_all
