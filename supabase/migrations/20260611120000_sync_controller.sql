-- Explicit timer controller for sync sessions.
-- Replaces open-mode multi-writer with a single controller_id.
-- Leader retains session admin (kick, end, visibility).

-- ── Column ──────────────────────────────────────────────────────
alter table public.sync_sessions
  add column if not exists controller_id uuid references auth.users (id) on delete cascade;

update public.sync_sessions
  set controller_id = leader_id
  where controller_id is null;

alter table public.sync_sessions
  alter column controller_id set not null;

-- ── RLS: drop open-mode participant writes ──────────────────────
drop policy if exists "Open mode participants update timer"
  on public.sync_sessions;

-- ── RLS: controller may update timer while session is active ────
drop policy if exists "Controller updates timer"
  on public.sync_sessions;
create policy "Controller updates timer"
  on public.sync_sessions for update
  using (
    auth.uid() = controller_id
    and status = 'active'
  )
  with check (
    auth.uid() = controller_id
...
-- ── Trigger: leader full access; controller timer fields only ───
...
begin
  ...
  -- Everyone else: no writes.
  raise exception 'Only the leader or controller may update the session';
end;
$$;

-- ── RPC: any active participant may take timer control ─────────
...
-- ── RPC: transfer leadership — move controller if old leader had it ─
...
-- ── RPC: leave — promote controller when controller leaves ───────
...
begin
  ...
  if v_session.leader_id = auth.uid() then
    ...
    -- Controller (non-leader) leaves — pass control to oldest remaining participant.
